...
class TestLLaVAAnalysisEndpoints:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("endpoint,request_method,data_key", [
        ("/api/v1/llava/analyze", "json", "json"),
        ("/api/v1/llava/analyze-upload", "files", "files")
    ])
    async def test_successful_analysis(self, httpx_mock: HTTPXMock, endpoint, request_method, data_key):
        ...
        assert data["description"] == "This image shows a person walking through a doorway."
        assert data["model_used"] == "llava:latest"
        assert data["processing_time"] > 0
        assert data["error_message"] is None

    @pytest.mark.asyncio
    @pytest.mark.parametrize("error_scenario,mock_setup", [
        ("service_unavailable", {"status_code": 503, "text": "Service unavailable"}),
        ("timeout", {"exception": httpx.TimeoutException("Request timed out")}),
        ("malformed_response", {"status_code": 200, "json": {}}),  # Missing expected fields
    ])
    async def test_error_handling(self, httpx_mock: HTTPXMock, error_scenario, mock_setup):
    ...
    @pytest.mark.parametrize("validation_scenario,request_data,expected_status", [
        ("missing_image", {"prompt": "Test"}, 422),
        ("empty_image", {"image_base64": "", "prompt": "Test"}, 200),  # Passes validation but may fail at Ollama
        ("invalid_image_type", {"image_base64": 123, "prompt": "Test"}, 422),
        ("invalid_prompt_type", {"image_base64": "dGVzdA==", "prompt": 123}, 422),
    ])
    def test_request_validation(self, validation_scenario, request_data, expected_status):
        ...
        if expected_status == 422:
            ...
            assert response.status_code in [200, 503]  # May succeed or fail depending on Ollama
    ...
    def test_file_upload_validation_scenarios(self):
        ...
        test_scenarios = [
            ...
            ("wrong_field_name", {"wrong_field": ("test.jpg", BytesIO(b"data"), "image/jpeg")}, 422),
        ]
        
        for scenario, files_data, expected_status in test_scenarios:
            with TestClient(app) as client:
                if files_data:
                    response = client.post("/api/v1/llava/analyze-upload", files=files_data)
                else:
    ...
    async def test_consistency_between_endpoints(self, httpx_mock: HTTPXMock):
        ...
        mock_response = {
            "response": "Consistent analysis result",
            "done": True
        }
        httpx_mock.add_response(
            method="POST",
    ...
    @pytest.mark.parametrize("file_scenario,filename,content,content_type", [
    ...
    def test_file_upload_types_and_edge_cases(self, file_scenario, filename, content, content_type):
    ...
    @pytest.mark.parametrize("prompt_scenario,prompt_value,expected_behavior", [
    ...
    def test_prompt_handling_variations(self, prompt_scenario, prompt_value, expected_behavior):
    ...
    async def test_concurrent_requests_handling(self, httpx_mock: HTTPXMock):
        ...
        with TestClient(app) as client:
            ...
            responses = []
            for i in range(3):
                response = client.post("/api/v1/llava/analyze", json={
                    "image_base64": test_image_base64,
    ...
    @pytest.mark.parametrize("size_scenario,image_size", [
        ("small", 1024),      # 1KB
    ...
    def test_image_size_handling(self, size_scenario, image_size):
        ...
        assert response.status_code in [200, 413, 503]  # Success, too large, or Ollama unavailable


class TestLLaVASecurityAndEdgeCases:
    """Test security aspects and edge cases."""

    @pytest.mark.parametrize("malicious_input,input_type", [
        ("../../../etc/passwd", "filename"),
    ...
    def test_security_input_handling(self, malicious_input, input_type):
        ...
        with TestClient(app) as client:
            if input_type == "filename":
                response = client.post(
                    "/api/v1/llava/analyze-upload",
                    files={"file": (malicious_input, BytesIO(b"test_data"), "image/jpeg")}
                )
            else:  # prompt
                response = client.post("/api/v1/llava/analyze", json={
                    "image_base64": test_image_base64,
    ...
    def test_performance_with_multiple_files(self):
        ...
        with TestClient(app) as client:
            for i in range(3):  # Reduced from 5 for efficiency
                ...
                response = client.post(
                    "/api/v1/llava/analyze-upload",
                    files={"file": (f"perf_test_{i}.jpg", BytesIO(small_data), "image/jpeg")}
                )
